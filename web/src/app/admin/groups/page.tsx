...
import { myGroups, publicGroups } from "@/lib/mock-data";
...
export default function AdminGroupsPage() {
  ...
  const demoMyGroups = myGroups.length;
  const demoPublic = publicGroups.length;

  const combined = useMemo(
    () => [
      ...myGroups.map((g) => ({ ...g, source: "meus_grupos" as const })),
      ...publicGroups.map((g) => ({ ...g, source: "publicos" as const })),
    ],
    [],
  );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-slate-500">
          Visão de negócio: quantos grupos existem no ecossistema, visibilidade e lotação. Hoje os
          dados são <strong className="text-slate-400">mock/demonstração</strong> no front; a
          evolução natural é listar grupos reais pela API NestJS + PostgreSQL.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-slate-800 bg-[#151b24] text-slate-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-400">Grupos “meus” (demo)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="font-display text-3xl font-bold text-white">{demoMyGroups}</p>
            <p className="text-xs text-slate-500">cadastro fixo para UX do app</p>
          </CardContent>
        </Card>
        <Card className="border-slate-800 bg-[#151b24] text-slate-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-400">Grupos públicos (demo)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="font-display text-3xl font-bold text-sky-400">{demoPublic}</p>
      ...
      <Card className="border-slate-800 bg-[#151b24] text-slate-100">
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-lg text-white">Catálogo demonstrativo</CardTitle>
          </div>
          <Button asChild variant="outline" size="sm" className="border-slate-600 text-slate-200">
            <Link href="https://peladapro.cloud/groups" target="_blank" rel="noopener noreferrer">
              Abrir app — Grupos
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {combined.map((g) => (
                  <tr key={`${g.source}-${g.id}`} className="border-b border-slate-800/80">
                    <td className="px-4 py-3">
                      <p className="font-medium text-slate-200">{g.name}</p>
                      <p className="text-xs text-slate-500">ID {g.id}</p>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-400">
                      {g.source === "meus_grupos" ? "Lista do usuário (mock)" : "Descoberta pública"}
                    </td>
